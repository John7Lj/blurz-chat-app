...
class TestGenerateHashedPassword:
    ...
    def test_truncates_password_at_72_bytes(self):
        from core.utils import generate_hashed_password, verify_password
        long_pw = "a" * 100
...
class TestAccessToken:
    ...
    def test_token_contains_user_data(self):
        from core.utils import access_token, decode_token
...
class TestCreationSafeLink:
    ...
    def test_de_serializ_url_expired_raises(self):
        ...
        link = CreationSafeLink("secret", "salt")
        token = link.create_safe_url({"email": "test@test.com"})
        time.sleep(2)  # Wait 2 seconds because itsdangerous uses integer timestamps
        with pytest.raises(TokenExpired):
            link.de_serializ_url(token, max_age=1)  # 1 second max age, token is 2s old

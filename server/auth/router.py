...
from core.utils import access_token, verify_password, CreationSafeLink, generate_hashed_password
...
async def logout(token: dict = Depends(AccessTokenBearer())):
    # Blacklist the token — we attempt it and always succeed from the client's
    # perspective (even if Redis is momentarily slow, the token is invalid after
    # the exp time anyway).
    try:
        await add_to_blacklist(token['jti'])
    except Exception:
        pass  # Non-critical: token expires naturally
    return JSONResponse(

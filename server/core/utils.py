...
from db.config import config
import uuid
from core.errors import InvalidToken,TokenExpired
import logging
...
def decode_token(token_data: str) -> dict | None:
    try:
        return jwt.decode(
            jwt=token_data,
            key=config.jwt_secret,
    ...
    except jwt.ExpiredSignatureError:          # must be BEFORE InvalidTokenError
        logging.warning("Token expired")
        raise TokenExpired()
    except jwt.InvalidTokenError as e:         # catches all other JWT errors
        logging.warning(f"Invalid token: {e}")
        raise InvalidToken()
    except Exception as e:
        logging.exception(f"Unknown error decoding token: {e}")
        return None                            # only truly unexpected errors reach here
